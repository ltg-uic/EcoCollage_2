...
globals [
  ...
  ;; The next foru are use by Tia's code for the SQL extension and to identify the run information  
  db
  query
  runID
  studyID
...
;  Repetitions-per-Iteration   ;; the number of subroutines per minute iteration. e.g, of this is set to 4, then each subroutine is 15 seconds. A subroutine is simple a full repetition of all the model processes.
  ...
  greatest-extent-lesser-flood
  greatest-extent-greater-flood
  ...
  Normalized-greatest-extent-lesser-flood
  Normalized-greatest-extent-greater-flood
...
to setup
  ...
  ifelse Flooding-Scenarios = "No outlet or inflows" [
    set activate-outlet? false
  ]
  [
    set activate-outlet? true 
  ]
  
  ifelse Flooding-Scenarios = "Outlet and Upstream flooding" or Flooding-Scenarios = "Outlet and upstream and river flooding" = true [
    set Upstream-flooding? true
  ]
  [
    set Upstream-flooding? false
  ]
  
  ifelse Flooding-Scenarios = "Outlet and river flooding" or Flooding-Scenarios = "Outlet and upstream and river flooding" = true [
    set Downstream-flooding? true
  ]
  [
    set Downstream-flooding? false
  ]
  
  ;;; Variables formerly sliders
  set Sewer-calibration-run? false
  set Inflow-calibration? false
...
;  set activate-outlet? true
  ...
  set Outlet-control-height 5 ;; in inches
...
;  set Repetitions-per-Iteration 3 ;; means 20 second subroutines
  ...
  set stop-tick ( storm-length + 2880 ) ;; setting the maximum length of a simulation is the surface water does not dry first. max length is 2 days beyond the end of a storm.
...
to Setup-GI
  ...
  set volume-a-swale 622.8 * conversion-multiplier ;; 82.28 ;; This is 622.8 mm max infil value of the engineered soil.
...
;          show GI-type-here
    ...
    ifelse remaining-budget >= 0 [
      set formatted-remaining-budget ( commify remaining-budget )
      set formatted-remaining-budget ( word "$" formatted-remaining-budget )   
      set budget-sentence word "Under budget by " formatted-remaining-budget
    ]
    [
      let abs-budget abs remaining-budget
      set formatted-remaining-budget ( commify abs-budget )
      set formatted-remaining-budget ( word "$" formatted-remaining-budget )   
      set budget-sentence word "Over budget by " formatted-remaining-budget
    ]
...
to go
  ...
  if ticks >= stop-tick [   ;;;; if ticks > storm-length and abs ( iteration-flow ) < 1 or ticks >= stop-tick [
    end-run
    stop
  ]
...
to flow-outlet-inlet
  ...
  if Upstream-flooding? = true and ticks >= inflow-start and ticks <= inflow-end [
    ask patches with [ inflow? = true ] [
      set iteration-inflow iteration-inflow + inflow-rate
      set water-column water-column + inflow-rate
      let inflow-volume ( inflow-rate * conversion-multiplier )
  ...
  ifelse Downstream-flooding? = false or ( Downstream-flooding? = true and ticks < inflow-start ) [
    ;;; Regular outflow
  ...
  [
    ...
    ifelse ticks <= inflow-end [
      ;; Flooding expands
      set downstream-head downstream-head + .5
    ]
    [
      ;; Recedes
      set downstream-head downstream-head - .5
      if downstream-head < 0 [ set downstream-head 0 ] ; Outlet-control-height-mm
    ]
    
    ask patches with [ outflow? = true ] [
      ifelse downstream-head > Outlet-control-height-mm [
        ;; If the river is higher than the outlet, then water is flowing in and out at will, so it is essentually a boundary condition
        let new-flood-water downstream-head - water-column
        set water-column downstream-head
        
        ifelse new-flood-water >= 0 [
          ;; water flowed in
          set iteration-inflow iteration-inflow + new-flood-water
          let inflow-volume ( new-flood-water * conversion-multiplier )
          set global-inflow ( global-inflow + inflow-volume )
          
        ]
        [
          ;; water flowed out
          set iteration-outflow iteration-outflow + ( abs new-flood-water )
          let outflow-volume ( ( abs new-flood-water ) * conversion-multiplier )
          set global-outflow ( global-outflow + outflow-volume )   
        ]
      ]
      [
        ;; Regular outflow happens i.e. the river has receeded enougn so that water flows out normally
        Outflow-water
      ]
    ]
  ]
...
to Outflow-water
  ...
  ask patches with [ outflow? = true ] [
    ...
    if Inflow-calibration? [ if outflow-now > 0 and global-outflow = 0 [ show ticks ] ]
...
to do-plots-and-globals
  ...
  ask patches [
    ...
    ;; Only applies to permeable cells
    if cover-type = 2 and water-column > highest-water-level [
      set highest-water-level water-column
    ]
  ...
  let high-water-now max [ water-column - storage-capacity ] of patches
  if high-water-now > greatest-depth-standing-water [ set greatest-depth-standing-water high-water-now ]
  let count-lesser-flood count patches with [ flooded? = true ]
  if count-lesser-flood > greatest-extent-lesser-flood [ set greatest-extent-lesser-flood count-lesser-flood ]
  let count-greater-flood count patches with [ extreme-flooded? = true ]
  if count-greater-flood > greatest-extent-greater-flood [ set greatest-extent-greater-flood count-greater-flood ]
...
to setup-outlet-inlet-values
  ...
  ;;; Inlets
  if Upstream-flooding? or Downstream-flooding? [
    set downstream-head Outlet-control-height-mm
    let potential-rain rainfall-rate * Repetitions-per-Iteration * storm-length * ( count patches )
    
    if Neighborhood-type = "Albany Park - Import" [
      set inflow-start 300
      set inflow-end 1771
      let duration-inflow inflow-end - inflow-start
      set inflow-rate ( potential-rain * .41 ) / duration-inflow

    ]
    
    if Neighborhood-type = "Blue Island - Import" [

      set inflow-start 752
      set inflow-end 1680
      let duration-inflow inflow-end - inflow-start
      set inflow-rate ( potential-rain * .4 ) / duration-inflow
    ]
    
    if Neighborhood-type = "Dixmoor - Import" [

      set inflow-start 695
      set inflow-end 1786
      let duration-inflow inflow-end - inflow-start
      set inflow-rate ( potential-rain * .39 ) / duration-inflow
    ]
    
    if Neighborhood-type = "Palos Heights - Import" [

      set inflow-start 706
      set inflow-end 1688
      let duration-inflow inflow-end - inflow-start
      set inflow-rate ( potential-rain * .37 ) / duration-inflow
    ]
  ]
...
to calculate-private-damages
  ...
  ;; Permeable cells calculate their damage amount based on the depth to the flood
  ask patches with [ cover-type = 2 and road? = false and alley? = false ] [
    ifelse highest-water-level >= patch-flooded-level [
      set depth-to-flood highest-water-level - 609.6 ;; 609.6 mm is 2 feet in mm ; this is the height of the first story
      ;; For structure depth damage ; assumign 2 or more stories with basement, single family; Table 2 in the PDF from John Watson; page 7 of the PDF
      ;; For content depth damagte ; assumign 2 or more stories with basement, single family; Table 5 in the PDF from John Watson; page 10 of the PDF
      let flood-depth-feet depth-to-flood / 304.8
      set flood-depth-feet round flood-depth-feet
;      show flood-depth-feet
      ifelse flood-depth-feet >= 10 [
        ;; 10 or greater. The tables go higher than this, but 10 is much higher than we would expect in these neighborhoods
        set percent-damage-structure random-normal 64.8 3.71
        set percent-damage-contents random-normal 34.4 3.04
      ]
      [
        ifelse flood-depth-feet = 9 [
          set percent-damage-structure random-normal 60.8 3.38
          set percent-damage-contents random-normal 31.7 2.76
        ]
        [
          ifelse flood-depth-feet = 8 [
            set percent-damage-structure random-normal 56.4 3.13
            set percent-damage-contents random-normal 29.1 2.56
          ]
          [
            ifelse flood-depth-feet = 7 [
              set percent-damage-structure random-normal 51.8 2.89
              set percent-damage-contents random-normal 26.7 2.36
            ]
            [
              ifelse flood-depth-feet = 6 [
                set percent-damage-structure random-normal 46.9 2.63
                set percent-damage-contents random-normal 24.3 2.15
              ]
              [
                ifelse flood-depth-feet = 5 [
                  set percent-damage-structure random-normal 41.9 2.34
                  set percent-damage-contents random-normal 22 1.92
                ]
                [
                  ifelse flood-depth-feet = 4 [
                    set percent-damage-structure random-normal 36.9 2.04
                    set percent-damage-contents random-normal 19.8 1.67
                  ]
                  [
                    ifelse flood-depth-feet = 3 [
                      set percent-damage-structure random-normal 31.9 1.75
                      set percent-damage-contents random-normal 17.7 1.43
                    ]
                    [
                      ifelse flood-depth-feet = 2 [
                        set percent-damage-structure random-normal 27 1.5
                        set percent-damage-contents random-normal 15.7 1.23
                      ]
                      [
                        ifelse flood-depth-feet = 1 [
                          set percent-damage-structure random-normal 22.3 1.35
                          set percent-damage-contents random-normal 13.8 1.11
                        ]
                        [
                          ifelse flood-depth-feet = 0 [
                            set percent-damage-structure random-normal 17.9 1.32
                            set percent-damage-contents random-normal 11.9 1.09
                          ]
                          [
                            ifelse flood-depth-feet = -1 [
                              set percent-damage-structure random-normal 13.9 1.37
                              set percent-damage-contents random-normal 10.1 1.13
                            ]
                            [
                              ifelse flood-depth-feet = -2 [
                                set percent-damage-structure random-normal 10.2 1.47
                                set percent-damage-contents random-normal 8.4 1.21
                              ]
                              [
                                ifelse flood-depth-feet = -3 [
                                  set percent-damage-structure random-normal 7.2 1.56
                                  set percent-damage-contents random-normal 6.8 1.29
                                ]
                                [
                                  ;; -4 or less. The tables go less than -4, but I am cutting it off here, since smaller values are nto possible than even -2 with this setup
                                  set percent-damage-structure random-normal 4.7 1.66
                                  set percent-damage-contents random-normal 5.2 1.37
                                ]
                              ]
                            ]
                          ]
      ...
      if percent-damage-structure < 0 [ set percent-damage-structure 0 ]
      if percent-damage-structure > 100 [ set percent-damage-structure 100 ]
      if percent-damage-contents < 0 [ set percent-damage-contents 0 ]
      if percent-damage-contents > 100 [ set percent-damage-contents 100 ]
      set average-percent-damage ( percent-damage-structure + percent-damage-contents ) / 2
;      show average-percent-damage
    ]
    [
      set percent-damage-structure 0
      set percent-damage-contents 0
      set average-percent-damage 0
    ]
  ]
end
...
to output-data
  ...
  set Cost-private-storm-damage mean [ average-percent-damage ] of patches with [ cover-type = 2 and road? = false and alley? = false ]
  
  let sewers-gallons global-sewer-ft3 * 7.48052 ;; 7.4 is number of gallons per cubic foot
  set Cost-public-storm-damage sewers-gallons * 0.00024 ;; cost to treat one gallon based on the user charge ordinance of MWRD
  ...
  set Normalized-Cost-install-private-GI Cost-install-private-GI / GI-Budget-dollars
  set Normalized-Cost-install-public-GI Cost-total-install / GI-Budget-dollars
  set Normalized-Cost-total-install Cost-total-install / GI-Budget-dollars
  set Normalized-Cost-private-maintenance Cost-private-maintenance / GI-Budget-dollars
  set Normalized-Cost-public-maintenance Cost-public-maintenance / GI-Budget-dollars
  set Normalized-Cost-total-maintenance Cost-total-maintenance / GI-Budget-dollars
  set Normalized-Cost-private-storm-damage Cost-private-storm-damage / 100 ;;; NOTE - this variable was in %, not $, so it was already normalized.
  set Normalized-Cost-public-storm-damage Cost-public-storm-damage / GI-Budget-dollars
...
;  set Normalized-time-to-dry time-to-dry / stop-tick
  ...
  set Normalized-greatest-extent-lesser-flood greatest-extent-lesser-flood / (count patches )
  set Normalized-greatest-extent-greater-flood greatest-extent-greater-flood / ( count patches )
  
;;; Normalized amoujnts of water

if Inflow-calibration? [
 show global-outflow / global-precipitation
  
]

  let normalization-denominator global-precipitation-ft3 * 4 
...
;show outflow-gallons
  ...
  set Normalized-greatest-depth-standing-water-ft greatest-depth-standing-water-ft / 3 ;; 2 feet? does this make sense?
  
  
  
;does not include greatest extent
  sql:configure "defaultconnection" [["host" "polarbear.evl.uic.edu"] ["port" 3306] ["user" "evl"] ["password" "CAT.evl"] ["database" "ecocollage"]]
  set query (word "INSERT INTO ecocollage.outFromSim (runID, studyID,  publicInstall, privateInstall,"
    " publicMaintain, privateMaintain,  publicDamage, privateDamage, runOff, waterInSewers, waterInAllGI, waterInRoofs,"
    "waterInRainBarrels, waterInSwales, waterInAlleys, greatestStandingWater, greatestDepthPuddle, timeToFlood,"
    "timeToLargeFlood, timeOfFlood, timeOfLargeFlood,"
    "publicInstallN, privateInstallN, publicMaintainN, privateMaintainN,  publicDamageN, privateDamageN, runOffN, waterInSewersN,"
    "waterInAllGIN, waterInRoofsN, waterInRainBarrelsN, waterInSwalesN, waterInAlleysN, greatestStandingWaterN,"
    "greatestDepthPuddleN, timeToFloodN, timeToLargeFloodN, timeOfFloodN, timeOfLargeFloodN,"
    " pathOfImage) VALUES ("runID", "studyID", "Cost-install-public-GI
    " , " Cost-install-private-GI", "Cost-public-maintenance", "Cost-private-maintenance", "
    Cost-public-storm-damage", "Cost-private-storm-damage", " global-outflow-ft3", " 
    global-sewer-ft3"," global-GI-total-ft3 " , " global-green-roof-ft3" , "global-rain-barrel-ft3", "global-swales-ft3"," 
    global-green-alleys-ft3 ", " global-standing-water-ft3 "," greatest-depth-standing-water-ft"," time-to-lesser-flood "," 
    time-to-greater-flood "," time-duration-lesser-flood", "time-duration-greater-flood", " 
    Normalized-Cost-install-public-GI " , " Normalized-Cost-install-private-GI", "Normalized-Cost-public-maintenance", "
    Normalized-Cost-private-maintenance " , " Normalized-Cost-public-storm-damage", " Normalized-Cost-private-storm-damage", " 
    Normalized-global-outflow-ft3", " Normalized-global-sewer-ft3"," global-efficiency-total" , " global-efficiency-green-roofs" , "
    global-efficiency-rain-barrels", "global-efficiency-swales "," global-efficiency-green-alleys ", " Normalized-global-standing-water-ft3 "," 
    Normalized-greatest-depth-standing-water-ft"," 
    Normalized-time-to-lesser-flood "," Normalized-time-to-greater-flood "," Normalized-time-duration-lesser-flood", " 
    Normalized-time-duration-greater-flood " , '"  imageName "')" )
  show query
...
data-visualized
data-visualized
"none" "flooding definitions" "accumulated water not including storage capacity-total water" "accumulated water not including storage capacity-relative colors" "cover type" "elevation"
1
...
895
489
Time to first 1" flood (hours)
time-to-lesser-flood / 60
2
1
11
...
895
538
Time to first 6" flood (hours)
time-to-greater-flood / 60
2
1
11
...
1141
592
Greatest amount of standing water (cubic feet)
global-standing-water-ft3
2
1
11
...
Flooding-Scenarios
Flooding-Scenarios
"Outlet but no inflows" "No outlet or inflows" "Outlet and Upstream flooding" "Outlet and river flooding" "Outlet and upstream and river flooding"
2
...
798
205
Export data if run stopped early
end-run
NIL
1
T
...
974
410
Normalized values (used in run score calculations)
13
0.0
1
...
1089
688
Greatest extent of 1" flood (% area)
Normalized-greatest-extent-lesser-flood * 100
0
1
11
...
1089
737
Greatest extent of 6" flood (% area)
Normalized-greatest-extent-greater-flood * 100
0
1
11
...
895
589
Duration of 1" flood (hours)
time-duration-lesser-flood / 60
2
1
11
...
895
638
Duration of 6" flood (hours)
time-duration-greater-flood / 60
2
1
11
...
#######
make sure slope is set to proper setting
#######
