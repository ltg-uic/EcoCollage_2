...
to setup
  ...
  set Starting-fullness-of-sewers .33
...
to setup-elevation
  ...
  ask patches [
    ...
    set elevation 350 + ( elevation-multiplier * ( Slope-percent / 100 ) * cell-dimension  ) ;; 350 as base because 150 subtracted for roads and 200 subtracted for swales
    set outflow? false
    set flooded? false
  ...
  ifelse Neighborhood-type = "Dense residential (e.g. Wicker Park)" or Neighborhood-type = "Bungalow belt" or Neighborhood-type = "Humboldt Park" or Neighborhood-type = "Albany Park"or Neighborhood-type = "Albany Park - Import"  or Neighborhood-type = "Random"[
    ...
    ask patches [
      ifelse  pxcor mod ( road-spacing-horizontal ) = 0 or pycor mod (   road-spacing-vertical ) = 0  or ( pxcor - 1 ) mod ( road-spacing-horizontal ) = 0 or  ( pycor - 1 ) mod (   road-spacing-vertical ) = 0 [
        ;; Road cell
        set road? true
        if curbs? = true [ set elevation elevation - 150 ]
        set pcolor grey
      ]
      ;; Not road
      [ set road? false ]   
...
to Setup-GI
  ...
  set volume-a-rain-barrel 2.27125 ;;0.227125 ;; volume of 60 gallon rain barrel in cubic meters
  set volume-a-green-roof 25.4 ;;2.54 ;; Volume of water that can be stored in 4 inches of soil on a green roof - assuming 1 inch (25.4 mm) of water can be stored.
  set volume-a-paver 231.618 ;; 23.1618 ;; Volume of water that can be stored in soil of GI - based on max infill value of 231.648 mm
  set volume-a-swale 822.8 ;; 82.28 ;; This is the 200 mm of surface storage + 622.8 mm max infil value of the engineered soil.
...
to setup-GI-permeable-paver
  ...
  if curbs? = true [ set elevation elevation - 75 ]
...
to setup-sewers
  ...
  ifelse Sewer-intake-regulator? = false [
    ...
    ;; Revises overall sewer rate into a per-sewer rate so that for a given landscape size, the total amount of water that could enter sewers is constant but the rate for sewer cells changes based on the proportion of sewer cells in the landscape
    ;; First, counts the sewer intake cells
    let sewer-count ( count patches with [ sewers? = true ] )
    ;; finds the % of cells that have sewer intakes
    let sewer-% ( sewer-count / (count patches ) )
    ;; sets a multiplier to be used in the next step
    let sewer-multiplier ( 1 / sewer-% )
    ;; takes the rate (mm per minute) and multiplies by the multiplier to see how to adjust the rate for the number of intake cells
    set sewer-rate ( 0.155340047 * sewer-multiplier )
    ;  type "number of sewers " print sewer-count
    ;  type "sewer rate " print sewer-rate
    ;; divides the sewer rate into a per iteration rate and rounds to the precision number of decimal places
    ;; this is the maximum intake rate for any sewer intake cell.
    set base-sewer-rate  ( sewer-rate / Repetitions-per-Iteration ) 
  ]
  [
...
;    ( ( accumulated-water + global-infiltration + global-evapo + global-sewer + global-outflow + water-in-storage ) / ( global-precipitation + global-inflow )) - 1
  if Upstream-bad-neighbor? = true and activate-outlet? = true and ( global-inflow < ( global-precipitation - ( accumulated-water + global-infiltration + global-evapo + global-sewer + water-in-storage ) ) ) [
    ask patches with [ inflow? = true ] [
      ...
      ask ? [
        if water-column > 0 or any? neighbors with [ water-column > 0 ] [ 
          ...
          foreach Neighbor-Elevation-list [
            ...
            if ticks >= storm-length [ set iteration-flow precision ( iteration-flow + flow ) 10 ]
...
to do-plots-and-globals
  ...
  set global-rain-barrel sum [ extra-GI-storage ] of patches with [ type-of-GI = "rain-barrel" ]
  set global-green-roof sum [ extra-GI-storage ] of patches with [ type-of-GI = "green roof" ]
  set global-green-alleys sum [ cumulative-infiltration-amount ] of patches with [ type-of-GI = "permeable paver" ]
  set global-swales ( sum [ cumulative-infiltration-amount ] of patches with [ type-of-GI = "swale" ] ) + water-in-storage
  set global-GI-total global-rain-barrel + global-green-roof + global-green-alleys + global-swales
  
  set total-extra-storage ( global-rain-barrel + global-green-roof ) * conversion-multiplier ;; green roofs and barrels
  ask patches [
  ...
  plot global-precipitation
  set-current-plot-pen "GI infiltration"
  plot global-GI-infiltration
  set-current-plot-pen "Permeable infiltration"
  plot global-Non-GI-infiltration
...
Neighborhood-type
Neighborhood-type
"Albany Park - Import" "Blue Island - Import" "Dixmoor - Import"
0
...
893
482
Volume of standing water (m^3), exclusing swale storage
non-GI-accumulated-water
2
1
11
...
consider revising the permeable soil - right now starts at about 10% air

Discuss:

1. variations on topography
