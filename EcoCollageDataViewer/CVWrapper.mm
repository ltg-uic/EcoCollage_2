...
+ (int) detectContours:(UIImage *)src corners:(int[]) cornersGlobal
{
    ...
    */
    ...
    int centerX = src_width / 2;
    int centerY = src_height / 2;
    
    for(i = 0; i < ptsNumber; i++) {
        //if (pts[i].x > 20 && pts[i].y > 20 && pts[i].x < (src_width - 20) && pts[i].y < (src_height - 20)) {
//        
//        double distanceFromCenterX = (double)(abs(centerX - pts[i].x));
//        double distanceFromCenterY = (double)(abs(centerY - pts[i].y));
//        double euclideanDistanceFromCenter = sqrt((distanceFromCenterX * distanceFromCenterX) + (distanceFromCenterY * distanceFromCenterY));
//
//        if (pts[i].x < centerX && pts[i].y < centerY && euclideanDistanceFromCenter < curr_dist0) { // QUADRANT 0
//            boardCorners[0] = pts[i];
//            curr_dist0 = euclideanDistanceFromCenter;
//        }
//        if (pts[i].x > centerX && pts[i].y < centerY && euclideanDistanceFromCenter < curr_dist1) { // QUADRANT 1
//            boardCorners[1] = pts[i];
//            curr_dist1 = euclideanDistanceFromCenter;
//        }
//        if (pts[i].x > centerX && pts[i].y > centerY && euclideanDistanceFromCenter < curr_dist2) { // QUADRANT 2
//            boardCorners[2] = pts[i];
//            curr_dist2 = euclideanDistanceFromCenter;
//        }
//        if (pts[i].x < centerX && pts[i].y > centerY && euclideanDistanceFromCenter < curr_dist3) { // QUADRANT 3
//            boardCorners[0] = pts[i];
//            curr_dist3 = euclideanDistanceFromCenter;
//        }
